...
By default, the index of the chromatin profile with the smallest value is plotted, but the users can optionally specify a target index with the \<index> argument. Furthermore, the plot title will indicate which target index was plotted but users can provide their own list of target names via with `--target_names` command. 
